                        ...
                        <div class="row" style="margin-top:30px">
                            <div class="col-lg-12 col-md-12 col-sm-12">
                                <ol class="breadcrumb breadcrumb-arrow">
                                    ...
                                    <li class="actiove"><span>Buku Pembantu</span></li>
                                </ol>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading"><h3 class="panel-title">Buku Pembantu</h3>
                                    </div>
                                    <div class="panel-body">
                                                ...
                                                <div class="form-group ">
                                                    ...
                                                    <div class="col-lg-4">
                                                        <div class="input-group">
                                                            <select id="nomor_rekening" onChange="cari_saldoawal();cari_rekening();" class="form-control">
                                        ...
                                        <div class="form-group pull-right">
                                            <div class="col-lg-12" style="margin-right: 15px;">
                                                <button class="btn btn-success waves-effect waves-light" type="button" id="btnbsr">Print</button>
                    ...
                    <div class="col-sm-12">
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                ...
                                <div class="form-group pull-right">
                                    <div class="col-lg-12" style="margin-right: 15px;">
                                        <button class="btn btn-success waves-effect waves-light" type="button" id="btnkcl">Print</button>
                                        <!--<button class="btn btn-success waves-effect waves-light" type="button">Simpan</button>-->
                                        <!-- <button class="btn btn-default waves-effect" type="button">Batal</button> -->
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    function exportExcelFile(workbook) {
        return XLSX.writeFile(workbook, "Buku Besar Pembantu.xlsx");
    }
    function exportExcelFiles(workbooks) {
        return XLSX.writeFile(workbooks, "Buku Besar Pembantu.xlsx");
    }
...
<script>
    ...
    function cari_rekening() {
        ...
        $.ajax({
            type: "POST",
            url: "index.php?page=e9fdc907eeadedf3373094ca4bdfb5a1&kode=965061478967c37fd4bd471a7091086e",
            ...
            success: function(msg) {
                ...
                $.each(JSON.parse(msg), function(idx, data) {
                    ...
                    /*$("#tbody").append('<tr class="tr" id="tr-'+data.id_detail_jurnal+'"><td class="td-'+data.id_detail_jurnal+'"><textarea readonly id="td-1-'+data.id_detail_jurnal+'" class="txt-'+data.id_detail_jurnal+'" name="" cols="5" rows="1" style="overflow:hidden;width: 100%; height:17px; background:none;border:none; resize:none;">'+data.tanggal_jurnal+'</textarea></td><td id="td-2-'+data.id_detail_jurnal+'" class="td-'+data.id_detail_jurnal+'">'+data.nomor_rekening+'</td><td class="td-'+data.id_detail_jurnal+'"><textarea readonly id="td-3-'+data.id_detail_jurnal+'" class="txt-'+data.id_detail_jurnal+'" name="" cols="5" rows="1" style="overflow:hidden;width: 100%; height:17px; background:none;border:none; resize:none;">'+data.uraian+'</textarea></td><td class="td-'+data.id_detail_jurnal+'"><textarea readonly id="td-4-'+data.id_detail_jurnal+'" class="txt-'+data.id_detail_jurnal+'"  name="" cols="5" rows="1" style="overflow:hidden;width: 100%; height:17px; background:none;border:none; resize:none;">'+data.nama_departemen+'</textarea></td><td class="td-'+data.id_detail_jurnal+'"><textarea readonly id="td-5-'+data.id_detail_jurnal+'" class="txt-'+data.id_detail_jurnal+'"  name="" cols="5" rows="1" style="overflow:hidden;width: 100%; height:17px; background:none;border:none; resize:none;">'+data.debit_jurnal+'</textarea></td><td class="td-'+data.id_detail_jurnal+'"><textarea readonly id="td-6-'+data.id_detail_jurnal+'" class="txt-'+data.id_detail_jurnal+'"  name="" cols="5" rows="1" style="overflow:hidden;width: 100%; height:17px; background:none;border:none; resize:none;">'+data.kredit_jurnal+'</textarea></td><td class="td-'+data.id_detail_jurnal+'"><textarea readonly id="td-7-'+data.id_detail_jurnal+'" class="txt-'+data.id_detail_jurnal+'"  name="" cols="5" rows="1" style="overflow:hidden;width: 100%; height:17px; background:none;border:none; resize:none;">'+data.nama_program+'</textarea></td></tr>');*/
                    sa = +sa + +$('#td-5-' + index).val();
                    //dbt = +dbt + +$('#td-5-'+data.id_detail_jurnal).val();
...
if ($('#status_rekening').text() === 'Aktiva') {
    ...
    var total_kredit = $('#total_kredit').val();
                    ...
                    //                    $('#saldo_akhir').val(+$('#saldo_awal').val() + +total_debit - +total_kredit);
                    // $('#saldo_akhir').val("-");
                    $('#mutasi').val((total_debit - +total_kredit).toFixed(2));
                    $('#saldo_akhir').val((0 + +total_debit - +total_kredit).toFixed(2));
                    if (+$('#saldo_akhir').val() < 0) {
                        $('#saldo_akhir').val((+$('#saldo_akhir').val() * -1).toFixed(2));
                    }
                    // $('#saldo_akhir').val(+$('#saldo_awal').val() + +total_debit - +total_kredit);
                    $('#mutasi').val((total_debit - +total_kredit).toFixed(2));
                    ...
                    //$('#saldo_akhir').val(+$('#saldo_awal').val() - sa);
                    //$('#mutasi').val(+$('#saldo_awal').val() - +$('#saldo_akhir').val());
                    //                    $('#saldo_akhir').val(+$('#saldo_awal').val() + +total_kredit - +total_debit);
                    // $('#saldo_akhir').val("-");
                    $('#mutasi').val((total_kredit - +total_debit).toFixed(2));
                    $('#saldo_akhir').val((0 + +total_kredit - +total_debit).toFixed(2));
                    if (+$('#saldo_akhir').val() < 0) {
                        $('#saldo_akhir').val((+$('#saldo_akhir').val() * -1).toFixed(2));
                    }
                    // $('#saldo_akhir').val(+$('#saldo_awal').val() + +total_debit - +total_kredit);

...
class buku_besar_model{
	...
	public function all(){
		// $sql0 = "SELECT SUBSTRING_INDEX(jurnal_detail.nomor_rekening, '.', 1) as nomor FROM jurnal_detail GROUP BY nomor ORDER BY nomor ASC;";
		// $statement = $this->connection->prepare($sql0);
		// $statement->execute();
		// $fetch = $statement->fetchAll();
		// $statement->closeCursor();
		// $sql1 = "SELECT SUBSTRING_INDEX(danamasuk_detail.no_rekening, '.', 1) as nomor FROM danamasuk_detail GROUP BY nomor ORDER BY nomor ASC;";
		// $statement = $this->connection->prepare($sql1);
		// $statement->execute();
		// $fetch1 = $statement->fetchAll();
		// $statement->closeCursor();

//		$sql0 = "SELECT nopokok_kelompokrekening as nomor FROM kelompokrekening GROUP BY nomor ORDER BY nomor ASC;";
//		$statement = $this->connection->prepare($sql0);
//		$statement->execute();
//		$fetch = $statement->fetchAll();
//		$statement->closeCursor();
...
//AND REPLACE(SUBSTRING_INDEX(nomor_rekening,'.',2),CONCAT(SUBSTRING_INDEX(nomor_rekening,'.',1),'.'),'') = REPLACE(SUBSTRING_INDEX(:nomor_rekening1,'.',2),CONCAT(SUBSTRING_INDEX(:nomor_rekening2,'.',1),'.'),'')
...
//AND REPLACE(SUBSTRING_INDEX(no_rekening,'.',2),CONCAT(SUBSTRING_INDEX(no_rekening,'.',1),'.'),'') = REPLACE(SUBSTRING_INDEX(:nomor_rekening1,'.',2),CONCAT(SUBSTRING_INDEX(:nomor_rekening2,'.',1),'.'),'')
...
WHERE
	jurnal_detail.nomor_jurnal = jurnal.nomor_jurnal
	AND
	 jurnal_detail.nomor_rekening = :nomor_rekening1
	AND
    SUBSTRING_INDEX(nomor_rekening, '.', 1) = kelompokrekening.nopokok_kelompokrekening
    AND
    jurnal.status_jurnal = 'Y'
...
WHERE
	danamasuk_detail.id_dana_masuk = danamasuk.id_danamasuk
AND 
    danamasuk_detail.no_rekening = :nomor_rekening1
AND
	danamasuk.status_danamasuk = 'Y' 
AND
    SUBSTRING_INDEX(no_rekening, '.', 1) = kelompokrekening.nopokok_kelompokrekening 
...
WHERE
	danakeluar_detail.id_dana_keluar = danakeluar.id_danakeluar
AND 
    danakeluar_detail.no_rekening = :nomor_rekening1
AND 
    SUBSTRING_INDEX(no_rekening, '.', 1) = kelompokrekening.nopokok_kelompokrekening 
AND 

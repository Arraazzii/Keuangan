...
class laporan_arus_kas_model{
    ...
    public function jenis_satu($tanggal_awal_kas, $tanggal_akhir_kas){
        $data = array();
        $saldo_akhir = array();
        $mutasi = array();
        $list_rekening = array();
        $sql1 = "SELECT pengaturanaruskas.koderekening, pengaturanaruskas.jenis, koderekening.uraian_koderekening, kelompokrekening.status_kelompokrekening FROM pengaturanaruskas, koderekening, kelompokrekening WHERE pengaturanaruskas.jenis = :jenis AND koderekening.koderekening = pengaturanaruskas.koderekening AND kelompokrekening.nopokok_kelompokrekening = koderekening.kelompok_koderekening;";
        $statement = $this->connection->prepare($sql1);
        $statement->bindValue(':jenis', 'Operasi');
        $statement->execute();
        $fetch1 = $statement->fetchAll();
        $statement->closeCursor();
        for($i = 0; $i < count($fetch1);$i++){
            $list_rekening = $this->cari_rekening($fetch1[$i][0], $tanggal_awal_kas, $tanggal_akhir_kas);
    ...
    public function jenis_dua($tanggal_awal_kas, $tanggal_akhir_kas){
        $data = array();
        $saldo_akhir = array();
        $mutasi = array();
        $list_rekening = array();
        $sql1 = "SELECT pengaturanaruskas.koderekening, pengaturanaruskas.jenis, koderekening.uraian_koderekening, kelompokrekening.status_kelompokrekening FROM pengaturanaruskas, koderekening, kelompokrekening WHERE pengaturanaruskas.jenis = :jenis AND koderekening.koderekening = pengaturanaruskas.koderekening AND kelompokrekening.nopokok_kelompokrekening = koderekening.kelompok_koderekening;";
        $statement = $this->connection->prepare($sql1);
        $statement->bindValue(':jenis', 'Investasi');
        $statement->execute();
        $fetch1 = $statement->fetchAll();
        $statement->closeCursor();
        for($i = 0; $i < count($fetch1);$i++){
    ...
    public function jenis_tiga($tanggal_awal_kas, $tanggal_akhir_kas){
        $data = array();
        $saldo_akhir = array();
        $mutasi = array();
        $list_rekening = array();
        $sql1 = "SELECT pengaturanaruskas.koderekening, pengaturanaruskas.jenis, koderekening.uraian_koderekening, kelompokrekening.status_kelompokrekening FROM pengaturanaruskas, koderekening, kelompokrekening WHERE pengaturanaruskas.jenis = :jenis AND koderekening.koderekening = pengaturanaruskas.koderekening AND kelompokrekening.nopokok_kelompokrekening = koderekening.kelompok_koderekening;";
        $statement = $this->connection->prepare($sql1);
        $statement->bindValue(':jenis', 'Penerimaan dari Bank, Jasa Giro & Deposito');
        $statement->execute();
        $fetch1 = $statement->fetchAll();
        $statement->closeCursor();
        for($i = 0; $i < count($fetch1);$i++){
    ...
    public function jenis_empat($tanggal_awal_kas, $tanggal_akhir_kas){
        $data = array();
        $saldo_akhir = array();
        $mutasi = array();
        $list_rekening = array();
        $sql1 = "SELECT pengaturanaruskas.koderekening, pengaturanaruskas.jenis, koderekening.uraian_koderekening, kelompokrekening.status_kelompokrekening FROM pengaturanaruskas, koderekening, kelompokrekening WHERE pengaturanaruskas.jenis = :jenis AND koderekening.koderekening = pengaturanaruskas.koderekening AND kelompokrekening.nopokok_kelompokrekening = koderekening.kelompok_koderekening;";
        $statement = $this->connection->prepare($sql1);
        $statement->bindValue(':jenis', 'Penghapusan/(Penambahan) Aktiva Tetap');
        $statement->execute();
        $fetch1 = $statement->fetchAll();
        $statement->closeCursor();
        for($i = 0; $i < count($fetch1);$i++){
    ...
    public function jenis_lima($tanggal_awal_kas, $tanggal_akhir_kas){
        $data = array();
        $saldo_akhir = array();
        $mutasi = array();
        $list_rekening = array();
        $sql1 = "SELECT pengaturanaruskas.koderekening, pengaturanaruskas.jenis, koderekening.uraian_koderekening, kelompokrekening.status_kelompokrekening FROM pengaturanaruskas, koderekening, kelompokrekening WHERE pengaturanaruskas.jenis = :jenis AND koderekening.koderekening = pengaturanaruskas.koderekening AND kelompokrekening.nopokok_kelompokrekening = koderekening.kelompok_koderekening;";
        $statement = $this->connection->prepare($sql1);
        $statement->bindValue(':jenis', 'Pendanaan');
        $statement->execute();
        $fetch1 = $statement->fetchAll();
        $statement->closeCursor();
        for($i = 0; $i < count($fetch1);$i++){
    ...
    public function jenis_enam($tanggal_awal_kas, $tanggal_akhir_kas){
        $data = array();
        $saldo_akhir = array();
        $mutasi = array();
        $list_rekening = array();
    ...
    public function jenisAll($tanggal_awal_kas, $tanggal_akhir_kas){
        $data = array();
        $saldo_akhir = array();
        $mutasi = array();
        $list_rekening = array();
    ...
    public function cari_rekening($nomor_rekening, $tanggal_awal_kas, $tanggal_akhir_kas){
        $sql0 = "SELECT
...
WHERE
    ...
    AND jurnal.tanggal_jurnal BETWEEN :tanggal_awal_kas AND :tanggal_akhir_kas ";
        $statement = $this->connection->prepare($sql0);
        $statement->bindValue(':nomor_rekening1',$nomor_rekening);
        $statement->bindValue(':tanggal_awal_kas',$tanggal_awal_kas);
        $statement->bindValue(':tanggal_akhir_kas',$tanggal_akhir_kas);
        $statement->execute();
        $fetch1 = $statement->fetchAll();
        $statement->closeCursor();
...
AND danamasuk.tanggal_danamasuk BETWEEN :tanggal_awal_kas AND :tanggal_akhir_kas;";
        $statement = $this->connection->prepare($sql1);
        $statement->bindValue(':nomor_rekening1',$nomor_rekening);
        $statement->bindValue(':tanggal_awal_kas',$tanggal_awal_kas);
        $statement->bindValue(':tanggal_akhir_kas',$tanggal_akhir_kas);
...
AND 
    ...
    AND danakeluar.tanggal_danakeluar BETWEEN :tanggal_awal_kas AND :tanggal_akhir_kas";
